...
CALL db.index.fulltext.createNodeIndex("namesAndCategories",["Product"],["name", "category"])
\end{lstlisting}

In dit voorbeeld gebruiken we de attributen \textit{name} en \textit{categories} van het nodetype \textit{Product}, en nemen we als zoekterm ``deodorant``.  Op deze manier kan er dus eenvoudig gekozen worden op welke velden van de objecten er moet vergeleken worden.
...
CALL gds.graph.create.cypher(
	...
	'MATCH (p2:Product)<-[r:BOUGHT|LIKES]-(c:Customer)-[:BOUGHT|LIKES]->(p3:Product)
	RETURN
		id(p2) AS source,
		id(p3) AS target,
		r.weight AS weight',
...
CALL gds.pageRank.stream('graph1', { maxIterations: 10, dampingFactor: 0.85 })
YIELD nodeId, score
RETURN gds.util.asNode(nodeId).name AS name, score
ORDER BY score DESC, name ASC
\end{lstlisting}

Het resultaat van dit algoritme (\autoref{fig:simplePageRankResult}) toont ons dat ``Toilet Paper`` de belangrijkste knoop is in deze graaf, met andere woorden dat als er door de graaf gelopen wordt, deze knoop de grootste kans heeft om bereikt te worden.
\begin{figure} [ht]
	\centering
